...
$(".default-language-delete-alert").on("click", function () {
    ...
    if ($(this).data("code")?.toString() === "en") {
        toastMagic.warning(
            messageContainer.data("title"),
            messageContainer.data("english")
        );
    } else {
        toastMagic.warning(
            messageContainer.data("title"),
...
$(".clean-database-form").on("submit", function (event) {
    ...
    if (checkboxes.length === 0) {
        ...
        toastMagic.warning(
            cleanDatabaseFormMsg?.data("warning"),
            cleanDatabaseFormMsg?.data("warning-msg"),
            true
        );
    }
});
...
$("#software-update-form").on("submit", function (event) {
    ...
    if ($('[name="update_file"]').val()) {
        ...
        let formData = new FormData(
            document.getElementById("software-update-form")
        );
        $.ajax({
            type: "POST",
