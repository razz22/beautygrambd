                ...
                <div class="card">
                    <div class="card-body">
                        <form action="{{route('admin.deal.update-data',[$deal['id']]) }}" method="post"
                              class="text-start onsubmit-disable-action-button form-advance-validation form-advance-inputs-validation form-advance-file-validation non-ajax-form-validate" novalidate="novalidate"
                                   ...
                                   <div class="h-100">
                                            ...
                                            @foreach($language as $lang)
                                                ...
                                                <div class="tab-pane fade {{ $lang == $defaultLanguage ? 'show active':'' }}" id="{{ $lang }}-form" role="tabpanel" aria-labelledby="{{ $lang }}-form-tab">
                                                    <div class="form-group">
                                                        ...
                                                        <label for="name" class="form-label">{{ translate('title') }}<span class="text-danger">*</span>
                                                            ({{strtoupper($lang) }})</label>
                                                        <input type="text" name="title[]" class="form-control" id="title"
                                                            value="{{ $lang==$defaultLanguage?$deal['title']:($translate[$lang]['title']??'') }}"
                                                            placeholder="{{ translate('ex').':'.' '.translate('LUX') }}"
                                                                {{ $lang == $defaultLanguage? 'required':'' }} data-maxlength="100" data-required-msg="{{ translate('title_field_is_required') }}">
                                        ...
                                        <div class="form-group mb-4">
                                            ...
                                            <input type="date" value="{{date('Y-m-d',strtotime($deal['start_date'])) }}"
                                                name="start_date" id="start-date-time" required data-required-msg="{{ translate('start_date_field_is_required') }}"
                                                class="form-control">
                                        </div>
                                        <div class="form-group">
                                            ...
                                            <input type="date" value="{{date('Y-m-d', strtotime($deal['end_date'])) }}"
                                                name="end_date" id="end-date-time" required data-required-msg="{{ translate('end_date_field_is_required') }}"
                                                class="form-control">
                                        </div>
                                   </div>
                                </div>
                                    ...
                                    {{ theme_root_path() == 'theme_aster' ? 'aster-flash-deal-disable-overlay' : '' }}">
                                        ...
                                        <div class="d-flex flex-column gap-30 w-100">
                                            ...
                                            <div class="upload-file">
                                                <input type="file" name="image" id="custom-file-upload" class="upload-file__input single_file_input"   data-max-size="{{ getFileUploadMaxSize() }}"
                                                    accept="{{getFileUploadFormats()}}|image/*"  value="" {{ $deal->banner_full_url ? '':'required' }} data-required-msg="{{ translate('image_field_is_required') }}">
                                                <div
